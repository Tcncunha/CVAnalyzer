...
- "keyword_analysis": Extract keywords from the job description and classify them.
  ...
  - "missing_keywords": keywords from the JD that are NOT present or evidenced in the candidate profile. Same shape. These represent gaps the candidate should address. Apply the semantic-matching rule below — literal phrasing differences are NOT gaps.
- SEMANTIC MATCHING (HARD REQUIREMENT): match MEANING, not exact wording. Paraphrases, synonyms, abbreviations, and demonstrated behavior all count as matches — search the ENTIRE profile (summary, skills, experience, education, certifications, languages) before declaring anything missing. Illustrative principle (apply generally): "fully available" satisfies an availability requirement; mentoring, supporting teams, or resolving client issues evidences a helpful attitude; adapting across environments or tools evidences willingness to learn; a tool/skill/certification listed in ANY section counts as present. List a keyword as missing ONLY when its meaning is genuinely absent from the whole profile. Never penalize the candidate for not using the JD's exact phrasing, and never report 0% for a category the JD doesn't require (omit it instead).
  - "category_scores": include ONLY categories that have at least one keyword in the JD. For each included category, compute (matched_count / total_in_jd_for_category) * 100, rounded to the nearest integer. If the JD contains zero keywords for a category, OMIT that category entirely — never score it 0, because 0 reads as candidate failure when the JD simply doesn't ask for it.
